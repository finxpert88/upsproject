...
function generateCurrent(seed,scenario,simTime,asset,context){
  ...
  if(scenario==='missing'){
    ...
    if(phase>=0&&phase<120){sourceTime=context.scenarioAt;communication=phase>30?'offline':'degraded';if(phase>30)for(const f of Object.keys(v))if(!['ratedCapacityKw','ratedCapacityKva','requiredRuntimeMinutes','lastTransferAt'].includes(f))qualities[f]='stale'}
  }
  return {at:simTime,sourceTime,scenario,values:v,qualities,communication,health:degraded?68:overload?88:96-asset.index};
}
function generateHistory(config,asset){const rows=[],interval=config.sampleIntervalSeconds*1000;for(let t=config.startAt-config.historyHours*3600000;t<=config.startAt;t+=interval)rows.push(generateCurrent(config.seed,config.initialScenario,t,asset,{startAt:config.startAt,scenarioAt:config.startAt}));if(rows.at(-1)?.at!==config.startAt)rows.push(generateCurrent(config.seed,config.initialScenario,config.startAt,asset,{startAt:config.startAt,scenarioAt:config.startAt}));return rows}
