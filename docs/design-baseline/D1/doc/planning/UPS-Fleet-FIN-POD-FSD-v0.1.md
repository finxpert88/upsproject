...
版本：v0.1 修订10｜日期：2026-09-15｜作者角色：产品规划与设计｜状态：按联合评审补充，待复审；修订9文档与DEMO通过记录保留，不自动延伸为本修订通过

本文定义功能、交互、数据契约、权限与验收要求；版本历史由Git追溯。

本轮变更见第23节及[修订10评审处置](UPS-Fleet-FSD-r10-review-disposition.md)。公开契约仍为schema2.0；schema2.1资产编辑方案独立列为候选，不因文档修改而自动启用。架构目录fsd-r9及其哈希继续代表修订9历史输入，不能标称已与本修订逐字同步。
...
本节补足第7、8、9、10、19节的实现约束，不新增FIN官方API或平台事务能力承诺。依据为[联合评审](../reviews/architecture-fsd-r9-joint-review.md)及[架构最终处置](../architecture/fsd-r9/review-disposition.md)。7项设计级关闭是既有架构复核结论；本修订尚待复审，后端实现与G1实测仍独立。

### 23.1 告警发生与恢复（AR-01）

每次检测发生须独立持久化身份、occurrenceOrdinal、previousOccurrenceId、detectedStartedAt和后续detectedEndedAt；结束事实采用追加事件或CAS更新，不能因身份不可变而无法记录恢复。RuleState只保存当前指针与水位，不能替代发生历史。每条CREATE/CLEAR命令保存不可变且可重放的载荷、目标发生、已知sourceAlarmId、前置命令及关联键，requestHash不能替代载荷。

ordinal分配、发生变化、RuleState水位及outbox入队具有同一原子提交边界；队列满或持久化失败不得前移水位。领取、发送、对账校验项目、模式、epoch与worker栅栏。未知结果先对账，后续发生不得绕过前置依赖；旧CLEAR仅结束旧发生对应的源告警。插件发生记录是检测与同步证据，不替代FIN源告警的活动/确认权威。目标FIN不能提供所需保证时，按第14节阻止该实现投产。

### 23.2 模拟重放状态（AR-02）

除公开SimSession外，内部须保存初始场景及版本、算法版本、seed、生成参数、逐设备场景分配和场景事件日志。每个事件包含targetSimEquipIds、virtualEffectiveAt、稳定eventSeq、参数及controlRevision，不能只用当前scenario恢复过去。

checkpoint记录已提交虚拟时间、已消费事件水位与分片恢复位置；它与对应样本、规则及告警结果同批提交，并带session/generation/worker栅栏。中断只重做未提交批次，不漏事件、不重复已提交事件。reset恢复原始生成初态并更换generation；清理覆盖场景事件、检查点、样本和关联模拟结果，保留第19.7要求的幂等/审计记录。旧worker迟到不得写入新代次。

### 23.3 配置版本重建（AR-04）

ConfigVersion为完整、不可变实体版本清单，引用不可变EntityVersion；清单包含确定排序的实体身份、entityRevision与内容hash。不同内容不得复用同一配置或实体版本号，同内容重试不产生新副作用。patch省略实体表示保留；删除必须显式声明并经依赖及授权检查，不能以缺字段推断删除。

完整清单及所有引用验证并持久化后才进入PREPARED/COMMITTED。Pointer恢复须验证清单、引用和hash并重建唯一全量集合；COMMITTED后发现损坏进入RECOVERY_REQUIRED，不能静默退回旧版。范围回退以当前获准scope生成新patch，保留无权范围及其他站点已提交内容。未决操作、历史解释或允许回退范围仍引用的版本不得回收。

### 23.4 编码与引用（AR-05）

每类运行记录须具有可机器校验的字段映射，声明wire字段、Rec标签、类型、必填/null、枚举、语义Ref路径及索引要求；不允许只依赖field到upsField的通用命名猜测。逻辑业务ID与Folio Ref由受控codec映射，JSON中的语义引用也须按声明路径递归重映射，普通文本不作全局替换。项目/模式/会话/generation来自可信上下文，解码须核对，不能信任载荷自报范围。

Enrollment、Rule、Capability和ReportJob须有wire→Rec→wire无损夹具；覆盖0、false、null、合法日期/时刻、Ref及列表。未知字段、非法枚举/日期和漏改的嵌套模板引用均须拒绝。Trio存在标签不代表平台已经建立唯一索引或事务能力。

### 23.5 幂等与模式授权（AR-06/07）

幂等唯一键固定为projectId＋operationId；actor、action、目标、请求体和原context是不可变绑定条件，不加入主键来允许重复执行。先检查当前身份与授权，再定位原操作并核对绑定；不同actor不得取得原记录，同actor变更绑定条件按既有冲突契约拒绝。create没有session时仍使用同一稳定主键；reset/delete旧generation重试仅可取得已登记原操作，不准借此执行新的旧代次副作用。OP-07按原动作、原mode及owner重新鉴权。

机器操作清单须覆盖第10.1全部模式矩阵：allowedContexts、permissionByMode、对象/会话范围、owner和动作蕴含。不能仅凭live权限字段生成授权器。simulation.read/ack/export/manage只授权对应模拟能力，不能获得真实确认或Telegram外发；OP-19/21/31的上下文限制沿原矩阵执行。

### 23.6 首次能力发现（AR-08）

GET /upsFleet/api/bootstrap为独立只读入口，可未登录调用，不要求schemaVersion、context或body；响应仅含supportedSchemaVersions和英文升级提示，不包含用户、权限、站点、设备或模拟会话。当前支持列表仅为["2.0"]。此项目路由不是FIN官方API名称。

完整能力POST仍要求登录、schema2.0和合法context；未登录为UNAUTHENTICATED，已认证但缺版本或版本不支持为SCHEMA_VERSION_UNSUPPORTED。bootstrap成功不授予业务权限，不返回完整能力DTO。

### 23.7 资产配置权威及候选范围（AR-03）

schema2.0内，Q唯一来源为该UPS当前配置的R-01.requiredMinutes；基准阻抗唯一来源为对应目标的R-03.baseline及comparableEvidenceRef。禁用规则可展示已配置参数并标Disabled，但不进行该规则评估；没有唯一合法参数则unknown，不能默认0。Dashboard、报告与规则按同一配置版本读取，禁止另建可编辑副本。环境关系仅可由已发布Mapping及真实点归属派生，不能直接写内部EnvironmentAssociation绕过发布。

额定容量、电池属性尚无schema2.0公开编辑payload，因此不得宣称插件内完整资产编辑已实现。建议采用[独立schema2.1候选](../architecture/schema2.1-candidate/README.md)及[候选类型](../architecture/schema2.1-candidate/contracts-proposed.ts)，增加assetProfile/environmentBinding并沿OP-08～11发布/回退；Q和阻抗不重复保存。候选已包含UPS/电池严格层级、逐字段证据有效期、null与删除不回退平台、依赖阻断、B推导required及同epoch投影。

待确定的产品影响是：迁移证据齐备才切换；旧2.0客户端仅兼容只读，确认/导出下载/暂停须升级；2.1配置生效后不能直接换回旧POD。候选还规定旧报表含新指标时不伪造兼容参数，以及电池属性证据到期逐字段置null并保留来源。此次“按评审修改文档”落实为候选完整登记，不记作用户已选择2.1迁移。正式采纳后须同步PRD追踪、本文件Entity/版本矩阵、生成器、validator与客户端；AR-03在此之前保持待决策。

### 23.8 补充验收定义与证据边界

|编号|对应项|判定条件|
|---|---|---|
|ARCH-01|AR-01|首次CREATE或CLEAR未知，后续多次发生后重启；原时间、ordinal及依赖可重建，旧CLEAR不结束新发生，入队失败水位不变|
|ARCH-02|AR-02|相同场景有效时间下，60秒一步与60次1秒、1/5/10倍事件及样本一致；提交前后中断无漏重，reset后旧worker被拒绝|
|ARCH-03|AR-03，候选|获批后验证资产编辑→校验→发布→回退、证据到期、跨范围拒绝、迁移门槛及旧客户端逐OP兼容；当前不标通过|
|ARCH-04|AR-04|A/B范围并发、显式删除、COMMITTED重启及范围回退唯一重建；不同内容覆盖旧版本、缺引用或hash不符被拒绝|
|ARCH-05|AR-05|四类对象无损往返与声明Ref重映射通过；漏改嵌套引用、非法类型/日期、额外字段负例失败|
|ARCH-06|AR-06|同ID改动作/目标/body、换actor、create重试及跨generation reset/delete均不产生第二副作用|
|ARCH-07|AR-07|模拟四类权限及live权限正反例通过，OP-07原动作owner拒绝保留，模拟不能真实确认/外发|
|ARCH-08|AR-08|无版本GET、缺版本POST、旧/有效版本、未登录矩阵可判定；bootstrap无业务信息|

以上8项为新增检查定义，不是本轮执行结果。既有架构参考夹具通过仅引用其原执行记录，不等于已在FIN部署。本轮FSD变化使旧validation JSON和fsd-r9基线hash成为历史证据；架构重取正文和重生成应独立复核，不修改旧证据来伪装匹配。G1继续要求实际FIN构建上的原子性/CAS/唯一性、告警生命周期与对账、历史追补、容量预算、secrets、文件及POD生命周期验证。
